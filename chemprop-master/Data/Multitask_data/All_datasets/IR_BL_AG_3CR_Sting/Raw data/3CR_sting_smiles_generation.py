...
all_components = pd.read_csv('4CR_Ketone_components.csv')
...
carboxys  = [tail for tail in all_components.carboxy_smiles[:10] if not amine == 'None']
amines  = [amine for amine in all_components.amine_smiles[:10] if not amine == 'None']


iso_names = [iso for iso in all_components.Isocyanide if not iso == 'None']
carboxy_names = [iso for iso in all_components.Carboxylic_acid if not carboxy == 'None']
ketone_names = [ket for ket in all_components.Ketone if not ket == 'None']
amine_names  = [amine for amine in all_components.Amine[:10] if not amine == 'None']
library_header = 'IR_4CR_Ketone_'


def generate_noncyclic_smiles(amine, iso, tail_1p1, tail_1p2, carboxy):
		...
		for k, ket in enumerate(ketone_names):
			for i, iso in enumerate(iso_names):
				for c, car in enumerate(carboxy_names):
					...
					tail_1p1 = tail1s[k]
					tail_1p2 = tail2s[k]
					full_smiles = generate_cyclic_smiles(amine_smiles, iso_smiles, tail_1_smiles, tail_2_smiles, car_smiles)
					all_lipid_dict['Ketone'].append(library_header + ket)
					all_lipid_dict['Isocyanate'].append(library_header + iso)
					...
					all_lipid_dict['Lipid_name'].append(library_header+ami+'_'+iso+'_'+tai)
...
generate_all_lipids().to_csv('Lipid_structures_with_fake_activities.csv',index = False)
