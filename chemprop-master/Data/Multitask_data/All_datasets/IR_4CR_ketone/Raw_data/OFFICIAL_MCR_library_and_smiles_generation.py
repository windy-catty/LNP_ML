...
all_components = pd.read_csv('OFFICIAL_secondary_mcr_components.csv')
...
iso_names = [iso_name for iso_name in all_components.Isocyanide if not iso_name == 'None']
amine_names = [amine_name for amine_name in all_components.Amine if not amine_name == 'None']
oh_names = [oh_name for oh_name in all_components.Term_oh if not oh_name == 'None']
# print(oh_names)
cooh_names = [cooh_name for cooh_name in all_components.Term_cooh if not cooh_name == 'None']
cooh_linker_names = [col_name for col_name in all_components.Cooh_linker if not col_name == 'None']
oh_linker_names = [ohl_name for ohl_name in all_components.Oh_linker if not ohl_name == 'None']
...
def component_name_split(name):
	if '&' in name:
		names = name.split('&')
		return names[0],names[1]
	return name, None

def generate_4cr_ketone_smiles_from_name(name):
	name = name.split(':')[1]
	coms = name.split('|')
	amine_name = coms[0]
	iso_name = coms[1]
	both_ohs = coms[2].split('+')
	oh1_name_1, oh1_name_2 = component_name_split(both_ohs[0])
	oh2_name_1, oh2_name_2 = component_name_split(both_ohs[1])
	cooh_name_1, cooh_name_2 = component_name_split(coms[3])

	amine = amines[amine_names.index(amine_name)]
	iso = isos[iso_names.index(iso_name)]
	if oh1_name_2 is None:
		# print(oh1_name_1)
		# print(oh_names)
		# print(ohs)
		# print(oh_names)
		oh1 = ohs[oh_names.index(oh1_name_1)]
		cooh_after_oh_1 = None
	else:
		oh1 = oh_linkers[oh_linker_names.index(oh1_name_1)]
		cooh_after_oh_1 = coohs[cooh_names.index(oh1_name_2)]

	if oh2_name_2 is None:
		oh2 = ohs[oh_names.index(oh2_name_1)]
		cooh_after_oh_2 = None
	else:
		oh2 = oh_linkers[oh_linker_names.index(oh2_name_1)]
		cooh_after_oh_2 = coohs[cooh_names.index(oh2_name_2)]

	if cooh_name_2 is None:
		cooh = coohs[cooh_names.index(cooh_name_1)]
		oh_after_cooh = None
	else:
		cooh = cooh_linkers[cooh_linker_names.index(cooh_name_1)]
		oh_after_cooh = ohs[oh_names.index(cooh_name_2)]
	return generate_4cr_ketone_smiles(amine, iso, oh1, oh2, cooh, oh_after_cooh = oh_after_cooh, cooh_after_oh_1 = cooh_after_oh_1, cooh_after_oh_2 = cooh_after_oh_2)

# oh_after_cooh = None, cooh_after_oh_1 = None, cooh_after_oh_2 = None)

def add_smiles_to_data_file(fname):
	df = pd.read_csv(fname)
	all_smiles = []
	mol_wts = []
	for lipid_name in df.Lipid_name:
		smiles = generate_4cr_ketone_smiles_from_name(lipid_name)
		all_smiles.append(smiles)
		ion_lipid_mol = Chem.MolFromSmiles(smiles)
		ion_lipid_mol_weight = Descriptors.MolWt(ion_lipid_mol)
		mol_wts.append(ion_lipid_mol_weight)
	df['smiles'] = all_smiles
	df['MolWt'] = mol_wts
	df.to_csv(fname[:-4]+'_with_smiles.csv',index = False)
...
def generate_all_cooh_linker_4cr_ketones(fraction = 2, fixed_number_to_screen = None):
	...
	for a, ami in enumerate(amines):
		...
		pd.DataFrame(amis).to_csv('Files_for_screen_v2/Completed_simple_symmetric_amines.csv',index = False)
				...
				for o1, oh1 in enumerate(ohs):
					for o2, oh2 in enumerate(ohs):
						if random.random()<fraction:
							...
							# name = '4K:' generate_4cr_ketone_smiles_name_and_components(a+1,i+1,o1+1,o1+1,cl+1, oh_after_cooh = o2+1)
							iso_name = iso_names[i]
							amine_name = amine_names[a]
							ketone_name = oh_names[o1]
							cooh_name = cooh_linker_names[cl]+'&'+oh_names[o2]
							name = '4K:'+amine_name+'|'+iso_name+'|'+ketone_name+'|'+cooh_name
...
def generate_screening_files(mol_dict,extra_x_df,fpaths):
	for i,row in extra_x_df.iterrows():
		path_if_none('Files_for_screen_v2/'+fpaths[i])
		pd.DataFrame({'SampleWeight':[1 for _ in range(len(mol_dict))]}).to_csv('Files_for_screen_v2/'+fpaths[i]+'/test_weights.csv', index = False)
		mol_dict.to_csv('Files_for_screen_v2/'+fpaths[i]+'/test_metadata.csv',index = False)
		mol_dict[['smiles','quantified_delivery']].to_csv('Files_for_screen_v2/'+fpaths[i]+'/test.csv',index = False)
		rowdf = pd.DataFrame({})
		for col in extra_x_df.columns:
			rowdf[col] = [row[col]]*len(mol_dict)
		# for j in range(len(mol_dict)):
		# 	rowdf = rowdf.append(row)
		rowdf.to_csv('Files_for_screen_v2/'+fpaths[i]+'/test_extra_x.csv',index = False)
...
def main(argv):
	...
	elif library_type == 'all_cooh_linker_4cr_ketones':
		if not library_generation_complete:
			library_df = generate_all_cooh_linker_4cr_ketones(fraction = fraction, fixed_number_to_screen = fixed_number_to_screen)
			# if not fixed_number_to_screen is None:
			# 	actual_length = len(library_df)
			# 	print('targeted: ',fixed_number_to_screen,' but actual made: ',actual_length)
			# 	library_df = generate_symmetric_cooh_linker_4cr_ketones(fraction = fraction, fixed_number_to_screen = float(fixed_number_to_screen)*float(fixed_number_to_screen)/actual_length)
			# 	print('now targeted: ',fixed_number_to_screen,' but actually made: ',len(library_df))
		library_df_name = 'All_cooh_linkers.csv'
		library_head = 'cooh_linker_4cr_k_'
	...
	if not library_type == 'reduce_library':
		...
		if library_generation_complete:
			library_df = pd.read_csv('Files_for_screen_v2/'+library_df_name)
		else:
			print(library_df.head())
			print(library_df_name)
			library_df.to_csv('Files_for_screen_v2/'+library_df_name, index = False)
		condition_names = list(pd.read_csv('Extra_x_files_v2/condition_names.csv')['Condition_name'])
		print(condition_names)
		fpaths = [library_head + con for con in condition_names]
		extra_x_df = pd.read_csv('Extra_x_files_v2/all_extra_xs.csv')
		generate_screening_files(library_df,extra_x_df,fpaths)		

if __name__ == '__main__':
	main(sys.argv)

# add_smiles_to_data_file('Experimental_4CR_ket_results/Ketone_experimental_results.csv')
