...
library_header = 'Zhou_dendrimer_'

def add_screen_data():
	structures = pd.read_csv('dendrimer_structures.csv')
	screen_df = pd.read_csv('dendrimer_screen_knockdown.csv')
	...
	for i, row in screen_df.iterrows():
		...
		for col in col_names:
			if not col=='Amine':
				...
				name = library_header + amine_number + '-' + col
				# try:
...
def generate_structures():
	...
	all_tail_nums = []
	all_mol_wts = []
	components = pd.read_csv('Zhou_dendrimer_components.csv')
...
add_screen_data().to_csv('dendrimer_sructures_with_activities.csv',index = False)
