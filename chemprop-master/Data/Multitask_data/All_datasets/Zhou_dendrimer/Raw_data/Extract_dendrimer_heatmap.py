...
header = 'dendrimer'
image = Image.open(header + '_heatmap.jpg')
pix = image.load()
# print(image.size)
# print(pix[64,64])
...
color_to_lum_dict = {}
color_to_lum_dict[255,246,206,255] = 0
color_to_lum_dict[251,203,150,255] = 1
color_to_lum_dict[243,105,33,255] = 2
color_to_lum_dict[241,37,26,255] = 3
...
x_start = 87
x_end = 810
y_starts = [55,98,177,244,399,465,520,574]
y_ends = [94,172,240,394,461,515,570,612]
values_high = [3,6,5,12,5,4,4,3]


all_vals = []
for i in range(len(values_high)):
	all_vals = all_vals + get_matrix_data([x_start,y_starts[i]], x_end-x_start, y_ends[i]-y_starts[i], 36,values_high[i])
...
df.to_csv(header + '_screen_knockdown.csv')
